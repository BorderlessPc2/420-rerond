...
exports.EVAL_FINDING_ANTI_NEEDLES = exports.EVAL_FINDING_NEEDLES = exports.EVAL_CASO_POR_TIPO = exports.EVAL_ASSERTIVIDADE_CASOS = void 0;
exports.resolveEvalCasoIdPorTipo = resolveEvalCasoIdPorTipo;
exports.scoreEvalCaso = scoreEvalCaso;
exports.scoreRelatorioContraEval = scoreRelatorioContraEval;
exports.EVAL_ASSERTIVIDADE_CASOS = [
    {
...
function resolveEvalCasoIdPorTipo(tipoAnaliseId) {
    if (!tipoAnaliseId)
        return null;
    return exports.EVAL_CASO_POR_TIPO[tipoAnaliseId] ?? null;
}
/** Needles por finding — relatório “bom” deve mencionar ao menos um. */
exports.EVAL_FINDING_NEEDLES = {
...
exports.EVAL_FINDING_ANTI_NEEDLES = {
...
function scoreEvalCaso(input) {
    const caso = exports.EVAL_ASSERTIVIDADE_CASOS.find((c) => c.id === input.casoId);
    if (!caso)
        return null;
 ...
 */
function scoreRelatorioContraEval(casoId, textoRelatorio) {
    const caso = exports.EVAL_ASSERTIVIDADE_CASOS.find((c) => c.id === casoId);
    if (!caso)
        return null;
    const blob = normalizeText(textoRelatorio);
    const findingsOk = [];
    for (const finding of caso.findings) {
        const needles = (exports.EVAL_FINDING_NEEDLES[finding.id] ?? [finding.item]).map((n) => normalizeText(n));
        const antis = (exports.EVAL_FINDING_ANTI_NEEDLES[finding.id] ?? []).map((n) => normalizeText(n));
        ...
        if (hitPositive && !hitAnti)
            findingsOk.push(finding.id);
    }
    return scoreEvalCaso({ casoId, findingsOk });
}
//# sourceMappingURL=evalAssertividadeCliente.js.map