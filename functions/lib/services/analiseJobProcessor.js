...
function buildDocumentoProjetoLabel(filename, arquivosMeta, url, sourceFilename) {
...
async function runAnaliseJob(params) {
    ...
    try {
        ...
        const { incluidos: pdfBuffersLimited, omitidos: pdfsOmitidos } = aplicarLimitesPdf(pdfBuffersRaw);
        lastStage = "extract";
        ...
        const pdfBuffers = await (0, pipeline_1.expandPdfsForContext)(pdfBuffersLimited, splitOpts);
        const splitNotices = [];
        ...
        for (const [src, count] of bySource) {
            splitNotices.push(`${src} (dividido em ${count} partes por tamanho/contexto — análise consolidada)`);
        }
        ...
        if (splitNotices.length > 0) {
            sharedParts.push((0, openaiService_1.buildTextInput)(`[AVISO: documentos grandes fatiados para caber no contexto: ${splitNotices.join("; ")}. Trate partes do mesmo arquivo como um documento contínuo na consolidação.]`));
        }
        ...
        const appendPdfParts = (parts, sourcePdfs) => {
            for (const pdf of sourcePdfs) {
                parts.push((0, openaiService_1.buildFileInput)(pdf.filename, pdf.buffer));
                parts.push((0, openaiService_1.buildTextInput)(buildDocumentoProjetoLabel(pdf.filename, arquivosMeta, pdf.url, pdf.sourceFilename)));
                const partLabel = (0, pipeline_1.buildPdfPartContextLabel)(pdf);
                if (partLabel)
                    parts.push((0, openaiService_1.buildTextInput)(partLabel));
            }
        };
        const runAnalyzeCall = async (parts) => (0, pipeline_1.withRetry)(() => (0, openaiService_1.analyze)(parts, {
         ...
         */
        const analyzePdfSetWithContextFallback = async (sourcePdfs, loteHint, depth = 0) => {
            const parts = [...sharedParts];
            if (escopoAnalise.incluirDocumentosProjeto) {
                appendPdfParts(parts, sourcePdfs);
            }
            if (loteHint)
                parts.push((0, openaiService_1.buildTextInput)(loteHint));
            ...
            try {
                ...
                console.warn(`Contexto estourado (depth=${depth}); fatiando ${sourcePdfs.length} PDF(s) e reanalisando…`, err instanceof Error ? err.message : err);
                ...
                if (sourcePdfs.length === 1) {
                    ...
                    for (let s = 0; s < finer.length; s++) {
                        ...
                        const sub = await analyzePdfSetWithContextFallback([item], `[RECORTE ${s + 1}/${finer.length} após overflow de contexto — consolidar com os demais.]`, depth + 1);
                        results.push(...sub);
                    }
                    return results;
                }
                ...
                for (let p = 0; p < sourcePdfs.length; p++) {
                    const sub = await analyzePdfSetWithContextFallback([sourcePdfs[p]], `[SUBLOTE ${p + 1}/${sourcePdfs.length} após overflow — um arquivo por chamada.]`, depth + 1);
                    results.push(...sub);
                }
                return results;
            }
        ...
        for (let i = 0; i < plannedBatches.length; i++) {
            ...
            const pdfsInBatch = batch.items.length > 0
                ...
                : escopoAnalise.incluirDocumentosProjeto
                    ? []
                    : [];
            const sourcePdfs = escopoAnalise.incluirDocumentosProjeto
                ? pdfsInBatch.length > 0
                    ? pdfsInBatch
                    : pdfBuffers
                : [];
            const loteHint = plannedBatches.length > 1 || sourcePdfs.some((p) => (p.partCount ?? 1) > 1)
